...
export async function deleteAdminTravelPlan(planId: string): Promise<{
  success: boolean;
  message?: string;
}> {
  try {
    const response = await serverFetch.delete(`/travel-plans/admin/${planId}`);
    ...
    console.error("Delete admin travel plan error:", error);
    return {
      success: false,
      message:
